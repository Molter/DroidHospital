...
public class ListaMedicamentosActivity extends Activity implements OnItemClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		edit_busca.addTextChangedListener(new TextWatcher() {
	        ...
	        public void onTextChanged(CharSequence s, int start, int before, int count) {
        		...
        		if (edit_busca.getText().toString().length() > 0){
        		...
        		if (start == 50){
     	    		Toast.makeText(getBaseContext(), "Limite do campo atingido." , Toast.LENGTH_SHORT).show();
 	     	    	edit_busca.setText(s.toString().substring(0, s.toString().length()-1));
	...
	private void setUpMedicamentos(String nome_medicamento) {
		try {
			...
			MedicamentoDescription interpretador = new MedicamentoDescription(nome_medicamento);
...
/*		Intent intent = new Intent(getApplicationContext(), ListaPacientesActivity.class);
		...
		// implementar retorno para tela de prescrição
*/	
	}
	
	private void atualizaLista(){

		MedicamentosAdapter adapter = new MedicamentosAdapter(this, medicamentos);
